  ...
  methods: {
    ...
    logout() {
      ...
      this.axios.get("/api/logout").then(({ data }) => {
        console.log('Logout response:', data); // 添加日志输出
        if (data.message === "登出成功") { // 确认返回的响应内容
          this.$store.commit("logout");
          console.log('User logged out successfully'); // 添加日志输出z
          this.$toast({ type: "success", message: "注销成功" });
        } else {
          this.$toast({ type: "error", message: data.message });
        }
      }).catch(error => {
        console.error('Logout error:', error); // 添加日志输出
        this.$toast({ type: "error", message: "注销失败，请稍后重试" });
      });
    }
  },
  computed: {
