      ...
      this.axios.post("/api/register", user).then(({ data }) => {
        if (data.flag) {
          ...
          this.axios.post("/api/login", param).then(({ data }) => {
            console.log("Login response data:", data); // 调试输出
            if (data.token) {  // 确保 token 存在
              this.$store.commit("login", data.user);
              this.$store.commit("setToken", data.token);  // 保存 token
              this.$store.commit("closeModel");
              this.$toast({ type: "success", message: "登录成功" });
            } else {
              this.$toast({ type: "error", message: "登录失败：Token 未返回" });
            }
          }).catch((error) => {
            console.error("Login error:", error);
            this.$toast({ type: "error", message: "登录请求失败，请稍后再试" });
          });
        } else {
          this.$toast({ type: "error", message: data.message });
        }
      }).catch((error) => {
        console.error("Registration error:", error);  // 捕获注册请求错误
        this.$toast({ type: "error", message: "注册请求失败，请稍后再试" });
      });
    }
