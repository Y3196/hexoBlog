      ...
      this.axios
          ...
          .then(({ data }) => {
            if (data.data.length) {
              ...
              // 使用Set来跟踪已经存在的文章ID
              const existingArticleIds = new Set(this.articleList.map(article => article.id));

              // 追加新数据到articleList时，排除重复的文章
              const newArticles = data.data.filter(item => !existingArticleIds.has(item.id));
              if (newArticles.length) {
                this.articleList.push(...newArticles);
                this.current++; // 只在有新数据追加时更新 current
                $state.loaded();  // 数据加载完成，继续显示光标
              } else {
                $state.complete();  // 没有更多数据时，隐藏光标
              }
            } else {
              $state.complete();  // 没有更多数据时，隐藏光标
            }
          ...
          .catch(error => {
            console.error(error);
            $state.complete();  // 请求失败时，隐藏光标以避免无限请求
          });
    }
  ...
  computed: {
    ...
    cover() {
      ...
      this.$store.state.blogInfo.pageList.forEach(item => {
        ...
        return "background: url(https://blog-felix.oss-cn-beijing.aliyuncs.com/articles/beijing2.jpg) center center / cover no-repeat";
    }
  }
};
</script>
