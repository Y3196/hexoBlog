...
export default {
  ...
  mounted() {
    this.article.isLiked = this.$store.state.articleLikeSet.includes(this.article.id.toString());
    console.log("Article isLiked status:", this.article.isLiked);
  },
  ...
  methods: {
    getArticle() {
      ...
      this.axios.get("/api" + this.$route.path).then(({ data }) => {
        ...
        // 更新视图，确保 viewsCount 正确显示
        console.log('Article viewsCount:', this.article.viewsCount);
        this.$nextTick(() => {
          // 判断当前文章是否被用户点赞
          this.article.isLiked = this.$store.state.articleLikeSet.includes(this.article.id);
          // 统计文章字数
    ...
    like() {
      ...
      // 发送请求
      this.axios.post(`/api/articles/${this.article.id}/like`, {}, {
        headers: {
          ...
          .then(({ data }) => {
            if (data.flag) {
              // 获取最新的 likeCount
              const newLikeCount = data.data; // data.data 是最终的 likeCount 数值
              this.$store.commit("articleLike", this.article.id); // 更新 Vuex 状态
              // 直接更新前端的 likeCount
              this.article.likeCount = newLikeCount;
              this.article.isLiked = this.$store.state.articleLikeSet.includes(this.article.id); // 确保 isLiked 是最新的
            } else {
  ...
  computed: {
    ...
    isLike() {
      // 确保 article.id 是字符串类型，因为 articleLikeSet 中的值是字符串
      const articleIdStr = this.article.id.toString();
      return this.$store.state.articleLikeSet.includes(articleIdStr)
          ? "like-btn-active"
          : "like-btn";
    },
    isFull() {
