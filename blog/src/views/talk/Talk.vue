...
export default {
  ...
  methods: {
      ...
      this.axios
          ...
          .then(({ data }) => {
            ...
            this.talkList.forEach(item => {
              // 检查每个 item 是否已经被点赞，如果是，将其加入 Vuex 的 talkLikeSet
              if (item.isLiked) {
                if (!this.$store.state.talkLikeSet.includes(item.id)) {
                  this.$store.commit('talkLike', item.id);
                }
              }
              if (item.imgList) {
    ...
    like(talk) {
      ...
      console.log('Authorization header:', headers);  // 打印 Authorization header
      ...
      this.axios.post("/api/talks/" + talk.id + "/like", null, { headers })
          ...
          .catch(error => {
            console.error('Error during the like request:', error);
          });
    }
