...
export default {
  ...
  methods: {
    getTalkById() {
      this.axios
          ...
          .then(({ data }) => {
            ...
            // 在页面加载时，确保更新 Vuex 中的点赞状态
            const talkIdStr = this.talkInfo.id.toString();
            if (this.talkInfo.isLiked) {
              if (!this.$store.state.talkLikeSet.includes(talkIdStr)) {
                this.$store.commit('talkLike', talkIdStr);
              }
            }
          })
          .catch(error => {
            console.error("Error fetching talk info:", error);
          });
    },
    ...
    like(talk) {
      ...
      this.axios.post(`/api/talks/${talk.id}/like`, null, { headers })
          .then(({ data }) => {
            if (data.flag) {
              this.$set(talk, "likeCount", data.data);
              // Toggle the like state in Vuex
              this.$store.commit("talkLike", talk.id.toString());
            } else {
              console.error("Failed to like the talk:", data.message);
            }
          })
          .catch(error => {
  ...
  watch: {
    '$store.state.talkLikeSet': {
      handler(newValue) {
        console.log('talkLikeSet changed:', newValue);
        // 这里可以根据新值更新组件的状态或执行其他操作
      },
      deep: true
    }
  },
  ...
  computed: {
    ...
    isLike() {
      return (talkId) => {
        ...
        return this.$store.state.talkLikeSet.includes(talkIdStr) ? "#eb5055" : "#999";
      };
    }
  }
};
