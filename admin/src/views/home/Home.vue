...
export default {
  created() {
    this.listUserArea();
    console.log('Component created'); // 添加调试信息
    this.getData();
  },
  data: function() {
    return {
  ...
  methods: {
    getData() {
      console.log('getData method called'); // 添加调试信息
      ...
      this.axios.get("/api/admin",{
        ...
        if (data.data.uniqueViewDTOList != null) {
          data.data.uniqueViewDTOList.forEach(item => {
            ...
            throw new Error('Manual error throw to check error handling'); // 手动抛出错误来检查
          });
        }
    ...
    listUserArea() {
      const token = this.$store.state.token || localStorage.getItem('token');
      console.log('Using token:', token); // 添加调试信息
      this.axios
        ...
        .then(({ data }) => {
          console.log('User area data:', data);
          this.userAreaMap.series[0].data = data.data;
          this.loading = false;  // 确保在数据加载后关闭加载状态
        }).catch(error => {
        console.error('Error loading user area:', error);
        this.loading = false;  // 即使加载失败也要关闭加载状态
      });
    }
  },
  watch: {
    type() {
      this.loading = true;  // 请求前设置为加载状态
      this.listUserArea();
    }
  }
};
